...
namespace Engine
{
	namespace Utilities
	{
		...
		void Interpolation::Spline(std::vector<double> &x, std::vector<double> &y, double yp1, double ypn)
		{
			...
			if (n <= 0 )
			{
				throw "n value in zero or less on spline" ; 				
			}
		...
		int Interpolation::ZeroFillMissing(std::vector<float> &x, std::vector<float> &y, int max_pts_toAddForZero)
		{
			...
			if (x.size() != y.size())
			{
				const char *mesg = "x and y need to be of the same size in ZeroFillMissing" ; 
			...
			if (num_pts <= 1)
			{
				throw "x is empty in ZeroFillMissing" ; 
				return 0 ; 
			}
		...
		int Interpolation::ZeroFillMissing(std::vector<double> &x, std::vector<double> &y, int max_pts_toAddForZero)
		{
			...
			if (num_pts <= 1)
			{
				throw "x size is 0 or less in ZeroFillMissing Overloaded" ; 
				return 0 ; 
			}
